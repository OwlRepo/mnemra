...
| `apps/api/src/chat/chat.controller.ts` | Workspace-scoped streaming chat endpoint that writes plain-text answer tokens and emits sources via `X-Chat-Sources` | Chat / RAG | Deep | Guarded by `JwtAuthGuard` + `WorkspaceMemberGuard`; DTO validation enforces non-empty `message` |
| `apps/api/src/chat/chat.service.ts` | Chat session/message persistence plus answer-cache orchestration for workspace-scoped streamed answers | Chat / RAG | Deep | Creates/reuses owned sessions, inserts user/assistant turns, checks exact then semantic cache, falls back to `answerQuestion()` on miss, and stores assistant sources/history identically for hits and misses |
| `apps/api/src/chat/chat.module.ts` | Wires chat controller/service and imports auth + cache dependencies | Chat / RAG | Standard | Imported by `app.module.ts` |
...
| `apps/api/src/chat/chat.service.spec.ts` | Unit coverage for exact-hit, semantic-hit, miss-to-cache, single-embed chat behavior | Chat / RAG | Standard | Mocks `@repo/ai` + `CacheService` |
| `apps/api/test/chat.e2e-spec.ts` | End-to-end RBAC + streaming + session/message history + cache-hit/invalidation coverage for workspace chat endpoints | Chat / RAG | Deep | Mocks `@repo/ai` so no live OpenAI call |
...
| `packages/ai/src/chains/index.ts` | Workspace-scoped RAG answer generation: one retrieval pass, source hydration from `documents`, then plain-text token streaming | Chat / RAG | Deep | `answerQuestion()` returns `{ sources, stream }`; `askQuestion()` remains back-compat wrapper |
| `packages/ai/src/chains/index.spec.ts` | Unit coverage for deduped source mapping and empty-retrieval fallback in chat chain | Chat / RAG | Standard | Mocks vector search, DB lookup, and LLM stream |
